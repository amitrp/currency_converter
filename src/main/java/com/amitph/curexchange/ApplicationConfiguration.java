...
public class ApplicationConfiguration {
    ...
    private void setCurrencyFormats() throws IOException {
        try (Stream<String> stream =
                Files.lines(Paths.get(currencyFormatsTxt.getFile().getPath()))) {
            currencyFormats =
    ...
    private void setCurrencyCrossReference() throws IOException {
        ...
        try (Stream<String> stream =
                Files.lines(Paths.get(currencyCrossReferenceTxt.getFile().getPath()))) {
            stream.map(s -> s.split("="))
                    .forEach(
                            arr ->
                                    currencyCrossReference.put(
                                            arr[0].substring(0, 3), arr[0].substring(3), arr[1]));
        }
    }
}
